...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\Bem vindo ao grupo *${mdata.subject}* respeite as regras e não seja ghost🐉`
				...
				teks = `óh o ban kklkkkk🥳 @${num.split('@')[0]} menos um`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
           ...
           return false
 	       ...
 	       if (isGroup) {
				try {
					...
					reply(`manda link não poha ${memberlimit}`)
					setTimeout( () => {
            ...
            if (bad.includes(messagesC)) {
				...
				if (messagesC.includes("://chat.whatsapp.com/")){
					...
					if (isGroupAdmins) return reply('Como cê é ADM então não vou te banir')
					client.updatePresence(from, Presence.composing)
					if (messagesC.includes("#izinadmin")) return reply("#permissão de administrador recebida")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
						reply(`Não manda link poha ${sender.split("@")[0]} agora vai tomar ban`)
						setTimeout( () => {
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("|")[1];
					const pref = `Use: \n${prefix}quotemaker texto|marca dágua\n\nEx :\n${prefix}quotemaker Meu ovo|Haruki`
                ...
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
              	    if (args.length < 1) return reply('cadê o texto, kralho?')
                    teks = `${body.slice(8)}`
               ...
               case 'help': 
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'Meu dono aekkkkk',MessageType.text, { quoted: mek} )
					break    
				case 'leaderboard':
				case 'lb':
				bo = args[0]
...
• Limit: ${limitawal-updated.limit}`
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'mining':
                      ...
                      if (!isEventon) return reply(`desculpe ${pushname} Os eventos não foram ativado pelo dono`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`Iae Haruki Gay, toma aqui essa esmola ${one}Xp `)
                      }else{
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`toma trouxa @${mentioned[0].split('@')[0]} perdeu adm otário`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marca quem vc quer promover!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
						...
						mentions(`Parabéns🥳 @${mentioned[0].split('@')[0]} *VOCÊ VIROU ADM,CARA!!!* não faça nada de errado para não perder este cargo!`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marca quem cê quer banir, burro!!!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
						...
						mentions(`ÓH O BAN KKKK @${mentioned[0].split('@')[0]} `, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨CESSO ❭ ATIVAR NSFW NESTE GRUPO')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
				...
				case 'clone':
			  	...
			  	case 'event':
					...
					if (Number(args[0]) === 1) {
						...
						reply('*❬ 𝗦𝗨C𝗘𝗦SO ❭ ATIVAR EVENTOS NESTE GRUPO*')
					} else if (Number(args[0]) === 0) {
						event.splice(from, 1)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ 𝗦𝗨CES𝗦O ❭ DESATIVANDO EVENTO NESTE GRUPO*')
					} else {
						reply(ind.satukos())
					}
					break 
				case 'antilink':
					...
					if (args.length < 1) return reply('Boo :𝘃')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*JÁ ESTÁ ATIVADO* !!!')
						antilink.push(from)
				...
				case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('FOTO DO GRUPO TROCADA COM SICESSO')
					break				
				case 'leave': 
				if (!isGroup) return reply(ind.groupo())
				if (!isOwner) return reply(ind.ownerb())
